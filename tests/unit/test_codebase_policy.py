...
Covers:
  ...
  - Daemon integration: handler scores first (Gate 1), then policy blocks (Gate 2)
"""
...
class TestDaemonPolicyCheck:
    ...
    def test_daemon_policy_check_escalate(self):
        """Write tool + read_only policy -> scored first, then ESCALATE by second gate."""
        from telos_governance.codebase_policy import CodebasePolicySigner
        ...
        # Escalated due to policy violation (second gate tightens)
        ...
        # Post-score architecture: hook IS called (Gate 1), then policy blocks (Gate 2)
        hook.score_action.assert_called_once()
