...
- Lyapunov-inspired energy functions for empirical convergence measurement
- Telic fidelity metrics (hard/soft)
- Basin membership testing

Note: Lyapunov stability analysis applies to the idealized continuous dynamical
system model. The actual TELOS system operates in discrete turns on LLM-generated
embeddings. V(x) is used as an empirical energy metric, not a formal stability proof.
...
class PrimacyAttractorMath:
    ...
    - Attractor center: a_hat = (t*p + (1-t)*s) / ||t*p + (1-t)*s||
    - Basin radius: r = 2/rho where rho = 1 - t
    - Lyapunov-inspired energy: V(x) = ||x - a_hat||^2 (empirical convergence metric)
    """
    ...
    def __init__(
        ...
        # Basin radius using inverse formula from Foundations: r = 1.0/rho
        ...
        # At t=0.9 (permissive), rho=0.25 gives r=4.0 (manageable)
        # At t=0.05 (strict), rho=0.95 gives r=1.053 (should catch real drift, allow on-topic)
    ...
    def compute_lyapunov_function(self, state: MathematicalState) -> float:
        """
        Compute Lyapunov-inspired energy V(x) = ||x - a_hat||^2

        Measures "energy" of state relative to attractor.
        Lower V -> closer to attractor -> better governance.

        Per Foundations: delta_V < 0 indicates empirical convergence
        toward the attractor. This is used as a measurement metric;
        formal stability guarantees apply to the idealized continuous
        dynamical system model, not the discrete-turn implementation.
...
class TelicFidelityCalculator:
    ...
    def compute_hard_fidelity(
        ...
        Per Foundations: F = (1/T) sum 1[x_t in B(A)]
    ...
    def compute_trajectory_stability(
        ...
        """
        Check if energy function is decreasing (empirically stable trajectory).

        Per Foundations: delta_V < 0 indicates empirical convergence
        toward the Primacy Orbit. Returns fraction of turns where V decreases.
