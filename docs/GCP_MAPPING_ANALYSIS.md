...
| 4 | **Immutable decision logs** | Ed25519-signed governance receipts on every decision. TKeys: AES-256-GCM encryption, HMAC-SHA512 signing, hash-chained audit logs. Article 72 EU AI Act aligned fields. | **FULLY MET** | `receipt_signer.py`, `crypto_layer.py`, `session.py`, 22/22 crypto verification tests |
...
- Claims 0% policy violations vs 26.67% prompt-based safety (different metric from TELOS 0% observed ASR)
...
| Claim | Number | Source |
|-------|--------|--------|
| Adversarial testing at scale | 2,550 attacks, 0% observed ASR (Cat A/B) | 7 benchmarks, 5,212 scenarios |
...
| SAAI alignment | 14 claims at 88% (self-assessed) | Machine-readable claims |
