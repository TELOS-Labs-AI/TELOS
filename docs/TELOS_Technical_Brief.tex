...
\textbf{Statistical Validity:} 95\% CI upper bound 0.12\% (Rule of Three); Clopper-Pearson exact: [0\%, 0.14\%]. Fisher's exact compared to baseline: $p < 0.0001$.
...
This is designed to support the record-keeping requirements of EU AI Act Articles 12/72, HIPAA Security Rule audit controls, and ISO 27001.
...
\begin{itemize}
    \item \textbf{Model coverage:} Validated with MiniLM embeddings (384-dim). The Governance Control Plane is model-agnostic (scores tool calls, not model outputs), but cross-embedding-model validation is ongoing.
...
TELOS achieved 0\% observed ASR across 2,550 attacks (95\% CI upper bound: 0.12\%) through mathematical enforcement in embedding space. XSTest validation shows an 8.0\% false positive rate with domain-specific configuration.

\textbf{Reproducibility:} Code, data, and validation scripts can be found at \url{github.com/TELOS-Labs-AI/telos}. All validation datasets published on Zenodo:
\begin{itemize}
    ...
    \item AgentHarm (352): 10.5281/zenodo.18564855
    \item AgentDojo (139): 10.5281/zenodo.18565869
    \item PropensityBench (977): 10.5281/zenodo.18562833
\end{itemize}
...
\midrule
Representation & Natural language & 384-dim vectors \\
