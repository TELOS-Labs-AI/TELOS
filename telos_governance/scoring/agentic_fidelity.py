...
class BoundaryCheckResult(NamedTuple):
    ...
    triggered_boundary_text: str = ""    # Text of the boundary that triggered (for audit)
    triggered_boundary_score: float = 0.0  # Cosine score of the triggering boundary
...
class AgenticFidelityResult:
    ...
    # Triggered boundary metadata (for audit trail -- Deliverable 2b)
    triggered_boundaries: List[str] = field(default_factory=list)  # Boundary texts that triggered
    boundary_scores: Dict[str, float] = field(default_factory=dict)  # Boundary text -> cosine score
...
class AgenticFidelityEngine:
    ...
    def score_action(
        ...
        # --- Composite Fidelity ---
        # Chain weight redistribution on first step: when there is no prior
        # chain context (previous_step is None, chain_continuity = 0.0),
        # the chain dimension contributes 0.15 * 0.0 = 0.0 -- dead weight.
        #
        # Two modes controlled by tc.chain_first_step_redistribute:
        #   False (default): only redistribute when purpose_sub_qualified
        #     (sub-centroid match). Preserves benchmark calibration.
        #   True (daemon mode): redistribute on ALL first-step events.
        #     Advisory fleet finding: 33% of live events lose 15% of
        #     composite to zero chain continuity on first-step/reset.
        w_purpose = self._tc.weight_purpose
        w_chain = self._tc.weight_chain
        _should_redistribute = (
            previous_step is None
            and chain_continuity == 0.0
            and (
                self._tc.chain_first_step_redistribute
                or purpose_sub_qualified
            )
        )
        if _should_redistribute:
        ...
        return AgenticFidelityResult(
            ...
            triggered_boundaries=(
                [boundary_result.triggered_boundary_text]
                if boundary_result.triggered_boundary_text else []
            ),
            boundary_scores=(
                {boundary_result.triggered_boundary_text: boundary_result.triggered_boundary_score}
                if boundary_result.triggered_boundary_text else {}
            ),
        )
    ...
    def _check_boundaries(
        ...
        return BoundaryCheckResult(
            ...
            triggered_boundary_text=worst_boundary.text if worst_boundary else "",
            triggered_boundary_score=max_violation,
        )
    ...
    def _make_decision(
        ...
        if effective_fidelity >= execute_thresh:
            ...
            # When chain weight is zero (demo mode), chain breaks have no
            # effect on decisions -- the dimension is structurally disabled.
            _chain_downgrade = chain_broken and self._tc.weight_chain > 0
            decision = ActionDecision.CLARIFY if _chain_downgrade else ActionDecision.EXECUTE
    ...
    def _identify_ambiguous_dimension(
        ...
        """
        if chain_broken and self._tc.weight_chain > 0:
