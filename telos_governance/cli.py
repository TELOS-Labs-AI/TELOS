...
@click.pass_context
def review(ctx):
    """Launch local review surfaces."""
    if ctx.invoked_subcommand is None:
        click.echo(_style("Available review surfaces:", bold=True))
        click.echo(f"  {'telos review vultr':<30s} Vultr governed cloud review app")
        click.echo()
        click.echo(_style("Run 'telos review <subcommand> --help' for options.", dim=True))


@review.command()
@click.option("--host", default="127.0.0.1", show_default=True, help="Host to bind the review app.")
@click.option("--port", default=8615, show_default=True, type=int, help="Port to bind the review app.")
def vultr(host, port):
    """Launch the Vultr governed cloud review app."""
    import subprocess

    app_path = _find_demo_script("vultr_local_review_app.py")
    if app_path is None:
        click.echo(_style("Error: demos/vultr_local_review_app.py not found", fg="red"))
        click.echo(_style("  Run from the TELOS project directory or reinstall with: pip install -e .", dim=True))
        sys.exit(1)

    env = dict(os.environ)
    env["TELOS_REVIEW_HOST"] = str(host)
    env["TELOS_REVIEW_PORT"] = str(port)
    if _NO_COLOR:
        env["NO_COLOR"] = "1"

    click.echo(_style("Launching Vultr governed cloud review app ...", fg="cyan"))
    click.echo(_style(f"  {app_path}", dim=True))
    click.echo(_style(f"  http://{host}:{port}", dim=True))
    click.echo()

    sys.exit(subprocess.call([sys.executable, str(app_path)], env=env))
