...
Architecture:
    Researcher query
        ...
        -> chain tracking   [Operationalsuspicious pattern detection]
...
def _detect_suspicious_patterns(chain: List[str]) -> List[str]:
    """Detect suspicious patterns from Operationalspecification.
    ...
    for i, s in enumerate(shorts):
        ...
        if consecutive_sweeps >= SWEEP_WARN_CONSECUTIVE:
            warnings.append(
                f"OperationalCLARIFY: {consecutive_sweeps} sweep calls without "
    ...
    if rescore_count >= RESCORE_WARN_CONSECUTIVE:
        warnings.append(
            f"OperationalCLARIFY: {rescore_count} rescore calls in session. "
    ...
    if inspect_count >= 5:
        warnings.append(
            f"OperationalCLARIFY: {inspect_count} inspect calls in session. "
    ...
    if export_count >= 3:
        warnings.append(
            f"OperationalESCALATE: {export_count} export calls in session. "
    ...
    if n > 0 and shorts[0] != "load" and shorts[0] != "validate":
        ...
        warnings.append(
            f"OperationalNOTE: First tool call was '{shorts[0]}', not 'load'. "
    ...
    if n >= MAX_TOOL_CALLS_WITHOUT_QUESTION:
        warnings.append(
            f"OperationalCLARIFY: {n} tool calls without documented research "
...
class TeloscopeInterpreter:
    ...
    def execute(self, query: str, **kwargs) -> str:
        ...
        5. Track chain continuity (Operationalpattern detection)
