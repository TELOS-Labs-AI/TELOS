...
# Traffic light thresholds (Report designgovernance health)
...
def _pct_with_denom(count: int, total: int) -> str:
    ...
    Report designprohibition #3: every percentage includes (n/N) format.
    """
    if total == 0:
        return "0.0% (0/0)"
    pct = count / total * 100
...
def _audit_trail_info() -> Tuple[str, int]:
    ...
    Report designprohibition #9: every report must reference teloscope_audit.jsonl.
...
def _negative_findings(
    ...
    Report designprohibition #8: negative findings must be reported prominently.
...
class ExecReport:
    ...
    def format(self) -> str:
        ...
        # Self-audit trail reference (Report designprohibition #9)
...
class MgmtReport:
    ...
    negative_findings: List[str]             # All negative findings (Report design#8)
    ...
    def format(self) -> str:
        ...
        # --- Negative findings (Report designprohibition #8) ---
        ...
        # --- TELOSCOPE self-audit (Report designprohibition #9) ---
        lines.append(_THIN_SEP)
...
class FullAuditReport:
    ...
    def format(self) -> str:
        ...
        # ================================================================
        # SECTION 1: FINDINGS REQUIRING ATTENTION (Report designprohibition #8)
        # ================================================================
        lines.append(_SEPARATOR)
        ...
        if self.escalate_events:
            ...
            lines.append(
                ...
                "or tool_args included per Report designSC-4.]"
            )
        else:
        ...
        # ================================================================
        # SECTION 12: TELOSCOPE SELF-AUDIT (Report designprohibition #9)
        # ================================================================
        lines.append(_SEPARATOR)
        ...
        lines.append(
            ...
            "trail per Report designprohibition #9.]"
        )
        lines.append("")
        lines.append(_SEPARATOR)
...
def executive_report(
    ...
    if validation_result is not None:
        ...
        # Report designSC-7: Never upgrade PARTIAL to PASS
...
def management_report(
    ...
    - Negative findings prominently displayed (Report designprohibition #8)
...
def full_audit_report(
    ...
    # ESCALATE event appendix -- Zone 1 data only (Report designSC-4)
    escalate_events = _build_escalate_appendix(corpus)

    # Survivorship disclosure (Report designprohibition #10)
...
def _build_escalate_appendix(corpus: AuditCorpus) -> List[Dict]:
    ...
    Report designSC-4: No request_text, no explanation, no tool_args.
...
def _build_survivorship_notes(corpus: AuditCorpus) -> List[str]:
    ...
    Report designprohibition #10: never present survivorship-biased results
...
def _build_bonferroni_notes(
    ...
    alpha = 0.05/6 = 0.0083 (Bonferroni correction for multiple comparisons).
    """
    notes = []
