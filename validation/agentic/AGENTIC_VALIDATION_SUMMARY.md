...
TELOS governance was validated against **three peer-reviewed agentic AI benchmarks** spanning 1,468 scenarios across tool selection, harm detection, and prompt injection detection. PropensityBench and AgentHarm achieved **100% Defense Success Rate (DSR)**. AgentDojo achieved **100% injection detection rate**, though overall task correctness was 52.2% due to generic PA scope mismatch (see AgentDojo note below).
...
| AgentDojo | ETH Zurich / NeurIPS 2024 | 139 | 100% injection detection* | [10.5281/zenodo.18565869](https://doi.org/10.5281/zenodo.18565869) |

\* **AgentDojo note:** 100% DSR refers to injection detection rate. Overall task correctness was 52.2% (benign task pass rate: 0%) because the generic PA used in evaluation was not calibrated for AgentDojo's specific task domains. The governance layer correctly identified all injections but also blocked most benign tasks due to scope mismatch. A domain-specific PA would be expected to improve benign pass rate.
...
- **139 injection scenarios** testing whether injected instructions can hijack agent behavior
